...
function getWebpackConfig (chain, cfg, { name, hot, cfgExtendBase = cfg.build }) {
  if (typeof cfgExtendBase.chainWebpack === 'function') {
    log(`Chaining ${name ? name + ' ' : ''}Webpack config`)
    cfgExtendBase.chainWebpack(chain)
  }
  ...
  if (typeof cfgExtendBase.extendWebpack === 'function') {
    ...
    cfgExtendBase.extendWebpack(webpackConfig)
  }
...
function getSSR (cfg) {
  ...
  require('./ssr/client')(client)
  require('./ssr/server')(server)
