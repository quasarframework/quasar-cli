...
module.exports = function (cfg, configName) {
  const
    ...
    fileHash = needsHash ? '.[hash:7]' : '',
    chunkHash = needsHash ? '.[chunkhash:7]' : '',
    resolveModules = [
  ...
  chain.module.rule('babel')
    ...
    .include
      ...
      .add(new RegExp('\\' + path.sep + 'quasar(-framework)?' + '\\' + path.sep))
      .end()
    .use('babel-loader')
      .loader('babel-loader')
        .options({
  ...
  // PRODUCTION build
    ...
    if (configName !== 'Server') {
      const
        ...
        regex = new RegExp('\\' + path.sep + '(node_modules|quasar)' + '\\' + path.sep)
