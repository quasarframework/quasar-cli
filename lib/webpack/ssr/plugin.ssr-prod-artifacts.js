  ...
  apply (compiler) {
    compiler.hooks.emit.tapAsync('ssr-artifacts', (compiler, callback) => {
      ...
      const
        ...
        ssrTemplate = compileTemplate(fs.readFileSync(ssrFile, 'utf-8'))({ opts: this.cfg.ssr })
      ...
      let pkg = {
        ...
        dependencies: Object.assign(appPkg.dependencies || {}, {
          ...
          'core-js': require(appPaths.resolve.cli('node_modules/core-js/package.json')).version,
          'lru-cache': deps['lru-cache'],
          'vue': deps.vue,
          'vue-server-renderer': deps['vue-server-renderer'],
          'vue-router': deps['vue-router'],
          'quasar-framework': deps['quasar-framework']
        }),
        engines: appPkg.engines
      }

      if (this.cfg.extras || this.cfg.animations) {
        pkg.dependencies['quasar-extras'] = deps['quasar-extras']
      }
      if (this.cfg.store) {
        pkg.dependencies.vuex = deps.vuex
      }

      if (typeof this.cfg.ssr.extendPackageJson === 'function') {
        this.cfg.ssr.extendPackageJson(pkg)
      }
      else {
        const deps = pkg.dependencies
        deps.express = '^4.0.0'
        deps.compression = '^1.0.0'
      }
