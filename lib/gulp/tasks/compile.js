...
var
  ...
  forceTheme = gulp._.config.app.forceTheme,
  blockTheme = forceTheme === 'mat' ? 'ios' : 'mat'
  ;
...
function compile(type, production) {
  ...
  if (forceTheme && type === 'css') {
    console.log(' !!!', ('[WARNING] Building one theme only: ' + forceTheme.toUpperCase()).underline.red);
    source = source.concat(['!src/css/app.' + blockTheme + '.styl']);
  }
