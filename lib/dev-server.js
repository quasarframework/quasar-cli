...
module.exports = class DevServer {
  ...
  listenSSR (webpackConfig, cfg, resolve) {
    const
      ...
      OuchErrorHandler = require('./helpers/cli-error-handling').switchToOuch()
    ...
    function render (req, res) {
      ...
      const handleError = err => {
        if (err.url) {
          ...
          OuchErrorHandler.handleException(err, req, res, output => {
    ...
    this.serverCompilerWatcher = serverCompiler.watch({}, () => {})

    // start building & launch server
    this.server = new WebpackDevServer(clientCompiler, Object.assign(
      {
        after: app => {
          if (cfg.ctx.mode.pwa) {
  ...
  stop () {
    ...
    return new Promise(resolve => {
      this.server.close(() => {
        ...
        if (this.serverCompilerWatcher) {
          this.serverCompilerWatcher.close(() => {
            this.serverCompilerWatcher = null
            resolve()
          })
        }
        else {
