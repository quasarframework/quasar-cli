...
function injectSsrInterpolation (templateHtml, hasMeta) {
  let html = templateHtml
    ...
    .replace(
      ...
      (found, start, end) => {
        ...
        return `${start} class="${classes.trim()}"${end}`
      }
    )
  ...
  if (hasMeta) {
    html = html
      .replace(
        /(<html[^>]*)(>)/i,
        (found, start, end) => `${start} %%Q_HTML_ATTRS%%${end}`
      )
      .replace(
        /(<head[^>]*)(>)/i,
      ...
      .replace(
        /(<body[^>]*)(>)/i,
        (found, start, end) => `${start} %%Q_BODY_ATTRS%%${end}%%Q_BODY_TAGS%%`
      )
  }
