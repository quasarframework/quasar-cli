...
function renameAppPage(oldName, newName) {
  ...
  /* istanbul ignore if */
  if (
    fs.exists(dest + '/css/style.' + oldName + '.styl') &&
    fs.move(dest + '/css/style.' + oldName + '.styl', dest + '/css/style.' + newName + '.styl')
  ) {
    return 1;
  }
  /* istanbul ignore if */
  if (
    fs.exists(dest + '/html/view.' + oldName + '.html') &&
    fs.move(dest + '/html/view.' + oldName + '.html', dest + '/html/view.' + newName + '.html')
  ) {
    return 1;
  }
  /* istanbul ignore if */
