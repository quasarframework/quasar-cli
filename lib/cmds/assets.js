...
function renameAsset(assetType, copyAsset, oldName, newName) {
  var
    returnValue = 0,
    src = fs.getPathToFolderFromApp('src/' + assetType + 's/' + oldName),
    dest = fs.getPathToFolderFromApp('src/' + assetType + 's/' + newName),
    operation = copyAsset ? 'copy' : 'move'
    ;

  /* istanbul ignore if */
  if (fs[operation](src, dest)) {
    return 1;
  }
...
function create(assetType, program, name) {
  ...
  /* istanbul ignore next */
  if (renameAsset(assetType, false, '___' + assetType + '___', name)) {
...
function rename(assetType, copyAsset, program, oldName, newName) {
  var
    src = fs.getPathToFolderFromApp('src/' + assetType + 's/' + oldName),
    dest = fs.getPathToFolderFromApp('src/' + assetType + 's/' + newName),
    title = assetType.charAt(0).toUpperCase() + assetType.slice(1)
  ...
  program.log.info((copyAsset ? 'Copying' : 'Renaming') + ' Quasar App ' + title + '...');

  /* istanbul ignore next */
  if (renameAsset(assetType, copyAsset, oldName, newName)) {
    program.log.error('Cannot ' + (copyAsset ? 'copy' : 'rename') + ' App ' + title + '.');
    return 1;
  }

  program.log.success('Quasar', title, oldName.green, copyAsset ? 'copyied to' : 'renamed to', newName.green, '.');
  return 0;
}
