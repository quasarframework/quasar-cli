...
class QuasarConfig {
  ...
  compile () {
    ...
    if (this.ctx.prod && this.ctx.mode.ssr) {
      cfg.ssr = merge({
        customServer: false,
        port: 3000,
        componentCache: {
      ...
      if (!cfg.ssr.customServer) {
        cfg.ssr.banner = {
          quasarVersion: require('../package.json').dependencies['quasar-framework']
        }

        if (
          cfg.ssr.https && (
            Object(cfg.ssr.https) !== cfg.ssr.https ||
            !cfg.ssr.https.key ||
            !cfg.ssr.https.cert ||
            !fs.existsSync(cfg.ssr.https.key) ||
            !fs.existsSync(cfg.ssr.https.cert)
          )
        ) {
          console.log()
          console.log(`⚠️  quasar.conf.js > ssr > "https" must be an Object`)
          console.log(`   with { key, cert } props, each pointing to`)
          console.log(`   a valid absolute path of a SSL key.pem and a cert.pem`)
          console.log()
          process.exit(1)
        }
      }
