  ...
  refresh () {
    ...
    if (this.ctx.dev) {
      ...
      if (this.opts.port) {
        ...
        if (initialPort) {
          warn(`Config port overriden by command line option`)
        }
      }
      else if (process.env.PORT) {
        cfg.devServer.port = process.env.PORT
        if (initialPort) {
          warn(`Config port overriden by PORT env variable`)
        }
      }

      if (this.opts.host) {
        cfg.devServer.host = this.opts.host
        if (initialHost && !this.ctx.mode.cordova) {
          warn(`Config host overriden by command line option`)
        }
      }
      else if (process.env.HOSTNAME) {
        cfg.devServer.host = process.env.HOSTNAME
        if (initialHost) {
          warn(`Config host overriden by HOSTNAME env variable`)
        }
      }

      if (cfg.devServer.port !== this.lastPort || cfg.devServer.host !== this.lastHost) {
        this.listenAdressChanged = true
        this.lastPort = cfg.devServer.port
        this.lastHost = cfg.devServer.host
      }
      else {
        this.listenAdressChanged = false
      }
